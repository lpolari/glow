...
namespace {
llvm::cl::OptionCategory category("adas-lrm Options");
...
Placeholder *loadNetworkModel(TypeRef inputType, Module *module,
                               unsigned int count) {
  Function *F = module->createFunction(netname + std::to_string(count));

  LOG(INFO) << "Loading " + netname + " model.";

  Caffe2ModelLoader loader("../data/onnx-models/" + netname + "/predict_net.pb",
                           "../data/onnx-models/" + netname + "/init_net.pb",
...
/// Starts a run of network on the given image. The image must be already
...
void dispatchClassify(unsigned int id, HostManager *hostManager,
  ...
  auto runid = hostManager->runNetwork(
      netname + std::to_string(id), std::move(context),
...
int main(int argc, char **argv) {
  ...
  input = loadNetworkModel(inputType, module.get(), 0);
  ...
  if (!tracePath.empty()) {
    traceContext->dump(tracePath, "network-trace");
    traceContext->saveToSQLite(netname);
  }
