...
bool GlowEnableJsonConfigPartiton = false;
...
static llvm::cl::opt<bool, /* ExternalStorage */ true>
    GlowEnableJsonConfigPartitonOpt(
    "glow_partitioner_enable_json_config",
    llvm::cl::desc(
        "Load partitions from json config"),
    llvm::cl::location(GlowEnableJsonConfigPartiton));
} // namespace glow
...
Expected<DAGListTy> Partitioner::loadBalancedPartition(CompilationContext &cctx,
  ...
  unsigned int totalRooflineTime = 0;
  for (auto &n : F_->getNodes()) {
    totalRooflineTime +=
        getNodeComputeTime(&n, backendMap_[deviceInfo_[0].backendName]);
  }

  int timePerPartition = totalRooflineTime / numDevices;
  LOG(INFO) << "TIME PER PARTITION" << timePerPartition << "!!!!!!!\n";
...
Partitioner::partitionFromConfig(const PartitionConfig &partitionConfig,
  ...
  if (partitionConfig.logicalIDs.size()) {
    ...
    // LPolariToDo Stopgap. Should actually use logical assigments from
    // json config. Why isn't it??
  } else {
  ...
  for (size_t i=0; i<partitionConfig.numOfPartitions; i++){
    partitions.at(0).nodes.at(i)->wcet90 = partitionConfig.wcet90[i];
    partitions.at(0).nodes.at(i)->wcet95 = partitionConfig.wcet95[i];

    /// for the MCDTS algorith, pessimistic wcet estimates are set to 0
    if (cctx.scheduling_policy == "MCDTS" && cctx.criticality == 0){
      partitions.at(0).nodes.at(i)->wcet100 = 0;
      partitions.at(0).nodes.at(i)->wcet105 = 0;
      partitions.at(0).nodes.at(i)->wcet110 = 0;
    } else {
      partitions.at(0).nodes.at(i)->wcet100 = partitionConfig.wcet100[i];
      partitions.at(0).nodes.at(i)->wcet105 = partitionConfig.wcet105[i];
      partitions.at(0).nodes.at(i)->wcet110 = partitionConfig.wcet110[i];
    }

    partitions.at(0).nodes.at(i)->timeslot_offset = i;
  }
...
Expected<DAGListTy> Partitioner::partitionFromJsonConfig(
    CompilationContext &cctx) {
  // Load partitioning information generated by the adas-pb tool
  // The filename is specified in the compilation context

  std::string file;
  file = cctx.partitionConfigFilename;
  ...
  PartitionConfig partitionConfig;
  partitionConfig.partitionNames = jf["partitionNames"]
                                       .get<std::vector<std::string>>();
  partitionConfig.backendNames = jf["backendNames"]
                                     .get<std::vector<std::string>>();
  partitionConfig.funcName = jf["funcname"];
  partitionConfig.numOfPartitions = jf["numOfPartitions"];
  partitionConfig.backendHints = {};
  partitionConfig.wcet90 = jf["wcet90"].get<std::vector<size_t>>();
  partitionConfig.wcet95 = jf["wcet95"].get<std::vector<size_t>>();
  partitionConfig.wcet100 = jf["wcet100"].get<std::vector<size_t>>();
  partitionConfig.wcet105 = jf["wcet105"].get<std::vector<size_t>>();
  partitionConfig.wcet110 = jf["wcet110"].get<std::vector<size_t>>();

  //  fill in network id
  partitionConfig.funcName.replace(partitionConfig.funcName.find("<id>"),
                                   4, std::to_string(cctx.network_id));
  std::for_each(partitionConfig.partitionNames.begin(),
                partitionConfig.partitionNames.end(),
                [&cctx](std::string &x){
    x.replace( x.find("<id>"),4, std::to_string(cctx.network_id));
  });

  llvm::StringMap<size_t> nodeToPartition;
  for (auto it: jf["nodeToPartition"].items()){
    nodeToPartition.insert(std::pair<llvm::StringRef,
                                     size_t>(it.key(),
                                             it.value()));
  }
  partitionConfig.nodeToPartition = nodeToPartition;

  // partition the network as defined in the partition config and return
  // the DAG of subnets
  auto partitions = partitionFromConfig(partitionConfig, cctx);
  return partitions;
}
...
Expected<DAGListTy> Partitioner::partition(CompilationContext &cctx) {
  ...
  if (glow::GlowEnableJsonConfigPartiton) {
    return partitionFromJsonConfig(cctx);
  }
